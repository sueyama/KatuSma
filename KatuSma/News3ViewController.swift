...
//  KatuSma
//
//  Created by 上山　俊佑 on 2018/05/09.
...
class News3ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIWebViewDelegate, XMLParserDelegate {
    ...
    override func viewDidLoad() {
        ...
        // 1つ進むボタン
        goButton = UIButton()
        goButton.frame = CGRect(x: self.view.frame.size.width - 50, y: self.view.frame.size.height - 120, width: 50, height: 50)
        ...
        // xmlを解析する
        let url:String = "https://rss.allabout.co.jp/aa/latest/ch/ch_bodycare/"
    ...
    func parser(_ parser: XMLParser, foundCharacters string: String) {
        if element == "title"{
            let arr1:[String] = string.components(separatedBy: "\n")
            titleString.append(arr1[0])
        }else if element == "link"{
            let arr2:[String] = string.components(separatedBy: "\n")
            linkString.append(arr2[0])
        }else if element == "description"{
            let arr3:[String] = string.components(separatedBy: "src=\"")
            let arr4:String!
            if arr3.count == 2 {
                arr4 = arr3[1].components(separatedBy: "\" border")[0]
            } else {
                arr4 = arr3[0]
            }
            if arr4.contains("http") {
                urlString.append(arr4)
                urlArray.append(urlString)
            }
        }
    }
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        ...
        if elementName == "item"{
            ...
            elements.setObject(urlString, forKey: "description" as NSCopying)
